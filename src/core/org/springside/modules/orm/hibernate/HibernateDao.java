...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	public Page<T> findPageWithSQL(final Page<T> page, final String sql, final Map<String, ?> values) {
		Assert.notNull(page, "page不能为空");
	...
	protected long countSqlResult(final String sql, final Object... values) {
		...
		try {
			Object count = findSqlUnique(countSql, values);
			if(count instanceof BigDecimal){
				BigDecimal Bcount =(BigDecimal) count;
	...
	protected long countHqlResult(final String hql, final Map<String, ?> values) {
		String countHql = "";
		try {
			countHql = StringUtils.substringBefore(hql, "order by");
			List list = find(countHql, values);
			return list.size();
		} catch (Exception e) {
			throw new RuntimeException("hql can't be auto count, hql is:" + countHql, e);
		}
